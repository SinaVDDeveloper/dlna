...
/*----------------------------------------------------------------------
|   PLT_MediaControllerTool add by linagyuanhuan, not use MediaServer obj
+---------------------------------------------------------------------*/

class PLT_MediaControllerTool
{
public:
	static NPT_Result ParseTimeStamp(const NPT_String& timestamp, NPT_UInt32& seconds)
	{
	    ...
	    // extract seconds
	    str.SubString(separator+1).ToInteger(value);//NPT_CHECK_WARNING(str.SubString(separator+1).ToInteger(value));
	    seconds = value;
	    str = str.Left(separator);
	    ...
	    // extract hours
	    str.ToInteger(value);//NPT_CHECK_WARNING(str.ToInteger(value));
	    seconds += 3600*value;
