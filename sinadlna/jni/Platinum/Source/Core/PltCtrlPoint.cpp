...
public:
    ...
    NPT_Result operator()(PLT_DeviceDataReference& device) const {
		/* BUG 有的service里action为空时候ready状态失败
		但是只要AVTransport service ok 即可
                                      ...
                                      PLT_DeviceDataReference&      device)
{
    ...
    if (root_device->GetType().Compare("urn:dial-multiscreen-org:device:dial:1") == 0) {
		NPT_LOG_INFO("AddDevice root_device->GetFriendlyName,%s",(const char*)root_device->GetFriendlyName());
